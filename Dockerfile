...
# 1. Установка базовых пакетов, PHP, Cron, Supervisor, OpenSSL и служебных утилит
RUN apt-get update && apt-get install -y \
    ...
    cron gettext logrotate bash openssl \
    && rm -rf /var/lib/apt/lists/*

# 2. Добавление официального репозитория Nginx и его установка
RUN curl https://nginx.org/keys/nginx_signing.key | gpg --dearmor \
    | tee /usr/share/keyrings/nginx-archive-keyring.gpg >/dev/null \
    && echo "deb [signed-by=/usr/share/keyrings/nginx-archive-keyring.gpg] \
    http://nginx.org/packages/debian bookworm nginx" \
    | tee /etc/apt/sources.list.d/nginx.list \
    && apt-get update && apt-get install -y nginx=1.30.0-1~bookworm \
    && rm -rf /var/lib/apt/lists/*
...
# 5. Создание самоподписанного сертификата (запасной вариант)
...
# ========== НАСТРОЙКА LOGROTATE И CRON ==========
COPY config/logrotate-nginx.conf /etc/logrotate.d/nginx
COPY config/crontab /etc/cron.d/nginx-ip-updates
RUN chmod 644 /etc/cron.d/nginx-ip-updates \
    && touch /var/log/cloudflare-update.log && chmod 666 /var/log/cloudflare-update.log
